...
% PELABELAN MATLAB code for pelatihan.fig
%      PELABELAN, by itself, creates a new PELATIHAN or raises the existing
%      singleton*.
%
%      H = PELABELAN returns the handle to a new PELATIHAN or the handle to
...
%
%      PELABELAN('Property','Value',...) creates a new PELATIHAN or raises the
...
% Last Modified by GUIDE v2.5 22-Nov-2019 20:31:17
...
for i = 1 : barisgb
    ...
    if jumlahhitam > 0
        ...
        for z = 1 : kolomgb
            arraybaris{jumlahbaris,1}(temp,z) = gambarbersih(i,z);
        end
        temp = temp+1;
    end
...
for a = 1 : barisak2
    for b = 1 : kolomak2
     ...
     if kosong == 1
       ...
       imwrite(arraykarakter{a,b},sprintf('IMG_%s.png', num2str(angka)),'BitDepth',1)
       angka = angka +1;
     end
end
msgbox('Proses Pelabelan Telah Selesai','Success','help');
